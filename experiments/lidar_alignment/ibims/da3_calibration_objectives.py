...
if not (
    ...
    raise RuntimeError(
        "GT, mask, DA3 and sparse map shapes do not match."
    )
...
np.save(
    OUTPUT_DIRECTORY
    / "lectureroom_01_da3_ls.npy",
    prediction_ls.astype(np.float32),
)

np.save(
    OUTPUT_DIRECTORY
    / "lectureroom_01_da3_log.npy",
    prediction_log.astype(np.float32),
)

np.save(
    OUTPUT_DIRECTORY
    / "lectureroom_01_da3_huber.npy",
...
figure.suptitle(
    "DA3 calibration objectives using the same 223 anchors"
)
...
visualization_path = (
    OUTPUT_DIRECTORY
    / "lectureroom_01_calibration_comparison.png"
)
