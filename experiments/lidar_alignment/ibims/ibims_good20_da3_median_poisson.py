...
def choose(rows: list[dict[str, str]]) -> list[dict[str, float | str]]:
    ...
    selected.sort(key=lambda row: (float(row["quality_score"]), float(row["all_valid_absrel_pct"])))
    return selected[:20]
...
def make_panel(rank, scene, rgb, gt, pred, valid, sparse, probes, selected_row, output):
    ...
    error_max = float(np.clip(np.percentile(err_show[np.isfinite(err_show)], 99.0), 0.5, 5.0))
...
def main():
    ...
    out_dir = metrics_path.parent / "examples_good20_da3_median_poisson"
    out_dir.mkdir(parents=True, exist_ok=True)
    save_csv(out_dir / "selected_good20.csv", selected)
    ...
    contact_sheet(panel_paths, out_dir / "00_good20_contact_sheet.png")
    print(f"\nMetrics source: {metrics_path}")
    print(f"Open: {out_dir / '00_good20_contact_sheet.png'}")
